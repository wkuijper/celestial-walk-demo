...
export function mesh(): Mesh {
    let nw = {pos:{x:-200,y:-200}} as Vertex
    let ne = {pos:{x:+200,y:-200}} as Vertex
    let se = {pos:{x:+200,y:+200}} as Vertex
    let sw = {pos:{x:-200,y:+200}} as Vertex
...
export function det3(a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number): number {
    return a*det2(e, f, h, i) - b*det2(b, f, g, i) + c*det2(d, e, g, h)
}

export function sq(s: number): number {
    return s*s
}

export function incircle(a: Vec2, b: Vec2, c: Vec2, d: Vec2) {
    let ad: Vec2 = minus(a, d)
    let bd: Vec2 = minus(b, d)
    let cd: Vec2 = minus(c, d)
    return det3(ad.x, ad.y, sq(ad.x)+sq(ad.y),
                    bd.x, bd.y, sq(bd.x)+sq(bd.y),
                    cd.x, cd.y, sq(cd.x)+sq(cd.y))
}
...
export function on_point(p1: Vec2, p2: Vec2) {
    let d: Vec2 = minus(p2, p1)
    return sq(d.x)+sq(d.y) < 0
}
...
export function approx_bisector_next(e2: HalfEdge): Line {
    ...
    return line_by_point_and_dir(e2.target.pos, rotate_left(minus(e3.target.pos, e2.origin.pos)))
}

export function obtuse_next(e2: HalfEdge): boolean {
    let e3: HalfEdge = e2.next
    return right_or_on_top_of(line_by_point_and_dir(e2.target.pos, rotate_right(minus(e2.target.pos, e2.origin.pos))), e3.target.pos)
}

export function gather_half_edges(m: Mesh): Set<HalfEdge> {
    let visited: Set<HalfEdge> = new Set();
    let waiting: [HalfEdge] = [] as [HalfEdge];
...
export function delete_edge(e: HalfEdge) {
    ...
    e.twin.prev.left = e.left
    e.twin.next.left = e.left
    // (face outgoing no longer matters, face is also deleted along with edge)
...
export function kink_left_next(e: HalfEdge): boolean {
    ...
    return strictly_right_of(line(e), e2.target.pos)
}
...
export function grow_edge(e: HalfEdge, p: Vec2) {
    ...
    let e3: HalfEdge = {origin: v, target: e.target, prev: e2, next: e} as HalfEdge
    e2.next = e3
    v.outgoing = e3
    ...
    let ee3: Edge = { half: e3 }; e3.edge = ee3; e2i.edge = ee3
    e3.twin = e3i
    e2i.prev = e3i
    e.next.prev = e2i
    e.prev.next = e3i
    let ff = { some: e2 } as Face
    e.left = ff; e2.left = ff; e3.left = ff
...
export function insert(m: Mesh, p: Vec2): Vertex {
